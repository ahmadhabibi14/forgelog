<script lang="ts">
  import {Columns3Cog, Container, LayoutDashboard, FileClock, type LucideProps, Settings, GlobeLock } from "@lucide/svelte";
...
<aside class="w-3xs h-full p-4 flex flex-col gap-1">
  {#each navmenu as nav (nav.path)}
    <a href={nav.path} class="flex flex-row gap-3 items-center py-2 px-2 hover:bg-neutral-900
      rounded-md active:ring-0 focus:ring-0 focus:border-none active:border-none
        focus:bg-neutral-900 focus:outline-0
        {$currentPage == nav.path ? 'bg-neutral-900' : ''}">
