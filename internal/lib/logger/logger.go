...
func (l *Logger) Trace(trace any, msg ...string) {
	...
	if configs.GetProjectEnv() == configs.EnvDev {
...
func (l *Logger) FatalIfErr(err error, msg ...string) {
	if err == nil {
		return
	}
	...
	if len(msg) > 0 {
		...
		l.logger.WithField(logFieldStck, stack).Fatal(err)
	}
}
...
func InitializeLogger() {
	...
	if configs.GetProjectEnv() != configs.EnvDev {
		var logOutput io.Writer = &rotateLogs.RotateLogsWriter{
